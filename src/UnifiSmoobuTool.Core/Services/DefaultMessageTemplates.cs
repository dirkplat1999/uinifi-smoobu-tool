...
/// <summary>Canonical default template bodies for the languages the app ships built-in
/// translations for (English, Dutch, German, French). Used both to seed a fresh database
/// (<see cref="Infrastructure.Persistence.SqliteConnectionFactory"/>) and to prefill a sensible
/// starting body when adding a template for one of these languages via the UI, so the two stay
/// in sync instead of drifting apart.</summary>
public static class DefaultMessageTemplates
{
    ...
    public static string? TryGetBody(string languageCode, MessageTemplateKind kind) =>
        All.FirstOrDefault(t =>
            string.Equals(t.LanguageCode, languageCode, StringComparison.OrdinalIgnoreCase) && t.Kind == kind)?.Body;
}
